...
bool EKF::ZeroAccelerationUpdate(
  unsigned int imu_id,
  double time,
  Eigen::Vector3d acceleration,
  ...
  Eigen::Matrix3d angular_rate_covariance)
{
  ...
  Eigen::MatrixXd H, sub_cov;
  unsigned int sub_size;

  if (m_state.imu_states[imu_id].get_is_intrinsic()) {
    bias_a = Eigen::Vector3d::Zero();
    bias_g = Eigen::Vector3d::Zero();
    sub_cov = m_cov.block<g_body_state_size, g_body_state_size>(0, 0);
    H = Eigen::MatrixXd::Zero(6, g_body_state_size);
  } else {
    bias_a = m_state.imu_states[imu_id].acc_bias;
    bias_g = m_state.imu_states[imu_id].omg_bias;
    unsigned int imu_index = m_state.imu_states[imu_id].index;
    unsigned int imu_size = m_state.imu_states[imu_id].size;
    unsigned int sub_size = g_body_state_size + imu_size;
    unsigned int bias_index;
    if (m_state.imu_states[imu_id].get_is_extrinsic()) {
    ...
    sub_cov.block<g_body_state_size, g_body_state_size>(0, 0) =
      m_cov.block<g_body_state_size, g_body_state_size>(0, 0);
    sub_cov.block(g_body_state_size, g_body_state_size, imu_size, imu_size) =
      m_cov.block(imu_index, imu_index, imu_size, imu_size);
    H = Eigen::MatrixXd::Zero(6, sub_size);
    H.block<3, 3>(bias_index + 0, bias_index + 0) = -Eigen::Matrix3d::Identity();
    H.block<3, 3>(bias_index + 3, bias_index + 3) = -Eigen::Matrix3d::Identity();
  }

  H.block<3, 3>(9, 9) = SkewSymmetric(
    m_state.imu_states[imu_id].ang_i_to_b.inverse() *
    m_state.body_state.ang_b_to_l.inverse() * g_gravity
  );
  ...
  if (score(0, 0) > motion_detection_chi_squared && m_is_gravity_initialized) {
    return false;
  }
  ...
  unsigned int imu_states_size = GetImuStateSize();
  ...
  m_state.body_state += body_update;
  m_state.imu_states += imu_update;
